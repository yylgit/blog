...
export const getNewOrderCount = createAction(types.GET_NEW_ORDER_COUNT, async(lastTime)=> {
    return await orderService.getNewOrderCount(lastTime || -1)
        .then(data=> {
            if (data == null) throw 'getNewOrderCount: data is empty';
            return data;
        });
}, (lastTime, resolved, rejected)=> {
    return {
        resolved,
        rejected
    }
...
```
export const testThunk = function () {
    // thunk 中间件处理 action是函数的情况
    return function (dispatch, getState) {
        //这里的dispatch就不走中间件了，原始的dispatch 只走reducer
